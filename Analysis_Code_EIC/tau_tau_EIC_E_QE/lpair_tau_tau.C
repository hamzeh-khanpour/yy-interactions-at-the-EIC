...
TFile *target;
TTree *Tsignal_LHeC = new TTree("EIC_E","EIC_E");
//TTree *Tsignal_LHeC = new TTree("EIC_QE","EIC_QE");
TFile *F;
...
// **********************************************************************
    ...
    TH1 *histMassdilepton =  new TH1F("M_{inv}", "", 100, 0.0, 100.0);
    TH1 *histPtdilepton   =  new TH1F("Pt", "",      10, 0.0, 10.0);
    TH1 *histq2           =  new TH1F("q2", "",      10, 0.0, 10.0);
    TH1 *histq2prime      =  new TH1F("q2prime", "", 10, 0.0, 10.0);

    TH1 *histetall         =  new TH1F("etall", "",     40, -20.0, 20.0);
    TH1 *histYll           =  new TH1F("Yll", "",       20, -10.0, 10.0);
    TH1 *histThetall       =  new TH1F("Thetall", "",   10, -5.0, 5.0);
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries; jentry++) {
      ...
      Float_t  integrated_cross_section_value_BH  = 32.93;   //   pb Elastic
//      Float_t  integrated_cross_section_value_BH  = 11.69;   //   pb   Q-Elastic
...
//      Yll = Etall -  TMath::Cos(Thetall)/2.0 * (Mtau/Ptll)* (Mtau/Ptll);
     ...
     target = new TFile ("EIC_E.root","recreate");
     target->cd();
...
TLatex *t2a = new TLatex(0.5,0.9,"#bf{EIC}");
                t2a->SetNDC();
                t2a->SetTextFont(42);
...
histMassdilepton->GetXaxis()->SetRangeUser(0,100);
